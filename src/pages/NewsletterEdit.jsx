...
import { getNewsLetter, updateNewsLetter } from '../services/newsletters';
...
export default function EditNewsletter() {
  ...
  useEffect(() => {
    const fetchNewsletter = async () => {
      try {
        const response = await getNewsLetter(id);
        ...
        setFilePreview(newsletter.fileUrl);
        setLoading(false);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    newsletterData.append('category', formData.category);
    if (formData.file) {
      newsletterData.append('file', formData.file);
    }

    try {
      const response = await updateNewsLetter(id, newsletterData);
        ...
        <div className="min-h-screen py-16 px-4 w-full flex justify-center items-center">
          <div className="max-w-4xl mx-auto w-full flex flex-col items-center justify-center">
            ...
            <div className="text-center mb-12">
              <h1 className="text-3xl font-bold text-gray-900 mb-3">Edit Newsletter</h1>
              <p className="text-gray-600 text-lg">Update your newsletter information</p>
            </div>
